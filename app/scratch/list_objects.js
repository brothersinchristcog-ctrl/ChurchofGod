...
async function listAllObjects() {
  try {
    ...
    // List All SObjects
    const listResp = await axios.get(`${instanceUrl}/services/data/v60.0/sobjects`, {
      headers: { Authorization: `Bearer ${sessionId}` }
    });

    const customObjects = listResp.data.sobjects
      .filter(o => o.custom)
      .map(o => o.name);
    
    console.log('\n--- 🛠️ CUSTOM OBJECTS IN ORG ---');
    console.log(customObjects.join(', '));

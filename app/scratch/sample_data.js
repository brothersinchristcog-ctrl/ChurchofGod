...
async function sampleData() {
  try {
    ...
    // Sample Daily_Sermon__c
    const dsResp = await axios.get(`${instanceUrl}/services/data/v60.0/query/?q=${encodeURIComponent('SELECT Name, YoutubeLink__c FROM Daily_Sermon__c LIMIT 5')}`, {
      headers: { Authorization: `Bearer ${sessionId}` }
    });
    console.log('\n--- 📋 DAILY SERMON SAMPLE ---');
    console.log(JSON.stringify(dsResp.data.records, null, 2));

    // Sample Daily_Video__c
    const dvResp = await axios.get(`${instanceUrl}/services/data/v60.0/query/?q=${encodeURIComponent('SELECT Video_Title__c, YouTube_ID__c FROM Daily_Video__c LIMIT 5')}`, {
      headers: { Authorization: `Bearer ${sessionId}` }
    });
    console.log('\n--- 📋 DAILY VIDEO SAMPLE ---');
    console.log(JSON.stringify(dvResp.data.records, null, 2));
